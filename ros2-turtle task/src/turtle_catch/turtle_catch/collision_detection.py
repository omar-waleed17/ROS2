...
class CollisionDetection(Node):
    def __init__(self):
        ...
        self.spawn_target_subscriber('turtle2')  # Assuming turtle2 is already spawned
    ...
    def check_collision(self):
        if self.base_pose:
            for turtle_name, target_pose in self.target_poses.items():
                ...
                if turtle_name == 'turtle2' and distance < 0.5:  # Check distance with turtle2
                    self.get_logger().info(f'Turtle {turtle_name} is too close! Teleporting...')
                    self.teleport_turtle(turtle_name)
                elif distance < 0.5:  # Threshold for "catching" other turtles
